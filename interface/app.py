...
# --- PANEL GÖRÜNÜM STATE'LERİ ---
if "show_file_panel" not in st.session_state:
    st.session_state["show_file_panel"] = True
if "file_panel_width" not in st.session_state:
    st.session_state["file_panel_width"] = 35  # Varsayılan Genişlik: %35
...
def init_agent():
    ...
    #ENHANCED_SYSTEM_PROMPT = SYSTEM_PROMPT + "\n\nÖNEMLİ: Kullanıcı bir kod, script veya içerik dosyası oluşturmanı istediğinde ya da bir kod ürettiğinde, bu içeriği ekrana yazmakla kalmayıp kesinlikle 'create_file' aracını (tool) çağırarak diske kaydetmelisin."
...
# --- 5. SOL SIDEBAR (OTURUM YÖNETİMİ VE ARAYÜZ AYARLARI) ---
...
# --- PANEL AYARLARI KONTROLÜ (SIDEBAR) ---
st.sidebar.markdown("---")
st.sidebar.subheader("📐 Ekran & Düzen Ayarları")

st.session_state["show_file_panel"] = st.sidebar.toggle(
    "📂 Sağ Dosya Panelini Aç", 
    value=st.session_state["show_file_panel"]
)

if st.session_state["show_file_panel"]:
    st.session_state["file_panel_width"] = st.sidebar.slider(
        "Sağ Panel Genişliği (%)", 
        min_value=20, 
        max_value=60, 
        value=st.session_state["file_panel_width"], 
        step=5,
        help="Panel çok dar kaldığında genişliği buradan artırabilirsiniz."
    )

active_session_id = st.session_state["current_session_id"]
config = {"configurable": {"thread_id": active_session_id}}

# --- 6. SAYFA DÜZENİ VE DİNAMİK KOLONLAR ---
if st.session_state["show_file_panel"]:
    file_ratio = st.session_state["file_panel_width"] / 100.0
...
with col_chat:
    # Üst başlık alanı ve panel aç/kapa hızlı butonu
    header_col1, header_col2 = st.columns([0.75, 0.25])
    with header_col1:
        st.title("🤖 Codebase RAG & Agent")
        st.caption(f"Aktif Session ID: `{active_session_id}`")
    with header_col2:
        btn_label = "📂 Paneli Kapat" if st.session_state["show_file_panel"] else "📂 Paneli Aç"
        if st.button(btn_label, use_container_width=True, type="secondary"):
            st.session_state["show_file_panel"] = not st.session_state["show_file_panel"]
            st.rerun()
...
if user_input := st.chat_input("Bir soru sorun veya dosya oluşturmasını isteyin..."):
    if user_input.strip() == "/compact":
        with st.status("compaction.md çalıştırılıyor...", expanded=True) as status:
            ...
            status.update(label="CONTEXT.md kaydedildi.", state="complete", expanded=False)
        st.rerun()
        ...
        # İlk mesajsa, kullanıcıyı hiç bekletmeden arka planda başlık üretmesi için thread başlatıyoruz
...
# --- SAĞ PANEL VE DOSYA İÇERİĞİ İÇİN ÖZEL CSS & YUKARI ÇIK BUTONU ---
st.markdown("""
    <style>
        /* Sağ panelin ekranla birlikte sabit kalması */
        ...
        /* Dosya içeriğinin kendi içinde scroll yapması */
        ...
        /* Kod bloklarının taşmasını engellemek */
        .stCodeBlock {
            max-height: 450px;
            overflow-y: auto;
        }
...
# === SAĞ TARAF: DİNAMİK VE AÇILIR/KAPANIR DOSYA PANENLİ ===
if col_files and st.session_state["show_file_panel"]:
    with col_files:
                ...
                if all_files:
                    ...
                    try:
                        ...
                        if ext in ["md", "txt", "markdown"]:
                            ...
                            with tab_preview:
                                st.markdown(
                                    f"<div class='file-scroll-container'>{file_content}</div>", 
                                    unsafe_allow_html=True
                                )
                            with tab_raw:
